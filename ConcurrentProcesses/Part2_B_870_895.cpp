...
void lockDB(){
    struct sembuf lockDB = {(short unsigned int) database_sem, -1, IPC_CREAT};

    while(semop(database_sem, &lockDB, 1) < 0){
        //cout << "in while loop" << endl;
    }
}

void unlockDB(){
    struct sembuf unlockDB = {(short unsigned int) database_sem, 1, IPC_CREAT};

    semop(key_sem, &unlockDB, 2);
} 
...
void Marking(int key) {
    for (unsigned int j = 0; j < database.size() * 3; j++){
        unsigned int index = j % database.size(); //Index to the next student
        lockKeys(key); //Attempt to grab 2 keys to be able to access the database
        lockDB();
        // Critical section starts here
        DataBaseCounter &currentStudent = DB[index]; //Access the database and grab the next student
...
int main() 
{
    ...
    database = ReadFile(filesystem::current_path().string() + "/" + "databaseShort.txt");
    ...
    database_sem = semget(IPC_PRIVATE , 5, 0666);

    if(key_sem == -1 || database_sem == -1) {
    ...
    semctl(database_sem, 0, SETVAL, 1);

    int shmid = shmget(IPC_PRIVATE , database.size() * sizeof(DataBaseCounter), 0666);
    if (shmid == -1) {
        perror("shmget failed");
        exit(1);
    }

    DataBaseCounter *DB = (DataBaseCounter *) shmat(shmid, NULL, 0); 
    ...
    for (int key = 0; key  < 5; key ++) {
        ...
        // Child process block
            ...
            cout << "TA" << key << " has started marking." << endl;
